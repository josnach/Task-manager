...
const verifyEmail = async (req, res) => {
  try {
    ...
    // Bug 1: condition was inverted — should return 401 IF expired (true), not if NOT expired
    if (isTokenExpired) {
    ...
    // Bug 2: verification is a document instance, not the model — use Verification.findByIdAndDelete
...
if (existingVerification && existingVerification.expiresAt > new Date()) {
  return res.status(400).json({
    message: "Reset assword request already sent",
  });
}
  if (existingVerification && existingVerification.expiresAt > new Date()){
    await Verification.findByIdAndDelete(existingVerification._id);
